...
export function NewProduct({ setOpenModalNewProduct, getAllProducts }) {
  ...
  const [inputUploadProductImage, setInputUploadProductImage] = useState();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const resp = await apiAddProduct(formInputs);
    if (resp?.success) {
      ...
      <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
        <div className="flex justify-between items-center pb-3">
          ...
          <div
            ...
            onClick={setOpenModalNewProduct}
          >
            <X />
          </div>
        </div>
        ...
        >
          ...
          <button className="px-3 py-2 bg-red-600 text-white mb-10 hover:bg-red-700">
            {isLoading ? "Espere" : "Guardar producto"}
          </button>
        </form>
      </div>
